...
--
-- Name: add_attendance_by_phone(integer, text, timestamp without time zone); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION add_attendance_by_phone(session_id_arg integer, student_id_arg text, time_arg timestamp without time zone) RETURNS boolean
    LANGUAGE plpgsql
    AS $$DECLARE
...
BEGIN
     ...
     IF (status_var = 'ONGOING' AND (student_id_arg NOT IN (SELECT person_id FROM attendance WHERE session_id = session_id_arg AND person_type = 'STUDENT'))) THEN
      ...
      INSERT INTO attendance (confirmed, session_id, time, person_id) VALUES(false, session_id_arg, time_arg, student_id_arg);
...
ALTER FUNCTION public.add_attendance_by_phone(session_id_arg integer, student_id_arg text, time_arg timestamp without time zone) OWNER TO postgres;
...
--
-- Name: add_grade_item2(text, integer, double precision, timestamp without time zone); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION add_grade_item2(name_arg text, grading_system_id_arg integer, total_score_arg double precision, timestamp_arg timestamp without time zone) RETURNS boolean
    LANGUAGE plpgsql
    AS $$DECLARE
...
ALTER FUNCTION public.add_grade_item2(name_arg text, grading_system_id_arg integer, total_score_arg double precision, timestamp_arg timestamp without time zone) OWNER TO postgres;
...
CREATE FUNCTION addstudentgrade(student_id_arg text, score_arg double precision, grade_item_id_arg bigint, section_id_arg integer) RETURNS boolean
    LANGUAGE plpgsql
    AS $$DECLARE
...
BEGIN
    ...
    SELECT INTO temp person_id FROM person_load WHERE person_id = student_id_arg AND section_id = section_id_arg;
     ...
     if temp isnull then
          ...
          INSERT INTO grade_item_entry (grade_item_id,score,person_id) VALUES (grade_item_id_arg, score_arg,student_id_arg);
...
CREATE FUNCTION delete_grade_item_entry(grade_item_entry_id_arg bigint) RETURNS boolean
    LANGUAGE plpgsql
    AS $$BEGIN

     DELETE FROM grade_item_entry WHERE id = grade_item_entry_id_arg;
...
--
-- Name: edit_gradeitem_totalscore(bigint, double precision); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION edit_gradeitem_totalscore(grade_item_arg bigint, total_score_arg double precision) RETURNS boolean
    LANGUAGE plpgsql
    AS $$BEGIN

     UPDATE grade_item SET total_score = total_score_arg WHERE id = grade_item_arg;
...
ALTER FUNCTION public.edit_gradeitem_totalscore(grade_item_arg bigint, total_score_arg double precision) OWNER TO postgres;
...
--
-- Name: edit_item_entry_score(bigint, double precision); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION edit_item_entry_score(gradeitementry_id_arg bigint, score_arg double precision) RETURNS boolean
    LANGUAGE plpgsql
    AS $$BEGIN

     UPDATE grade_item_entry SET score = score_arg WHERE id = gradeitementry_id_arg;
...
ALTER FUNCTION public.edit_item_entry_score(gradeitementry_id_arg bigint, score_arg double precision) OWNER TO postgres;
...
--
-- Name: enrolled_student_names(bigint); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION enrolled_student_names(section_id_arg bigint) RETURNS SETOF text
    LANGUAGE plpgsql
    AS $$DECLARE
...
BEGIN
 ...
 FOR student_names IN SELECT person.id || '#' || person.last_name || ', ' || person.first_name FROM person,person_load WHERE person_load.section_id = section_id_arg AND person_load.person_id = person.id AND person_load.person_type = 'STUDENT' ORDER BY person.last_name   LOOP

 RETURN NEXT student_names;
...
ALTER FUNCTION public.enrolled_student_names(section_id_arg bigint) OWNER TO postgres;
...
--
-- Name: get_children_names(text); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION get_children_names(parent_id_arg text) RETURNS SETOF text
    LANGUAGE plpgsql
    AS $$DECLARE
...
BEGIN
 ...
 FOR children_names IN SELECT person.id || '#' || person.last_name || ', ' || person.first_name FROM person,linked_account WHERE linked_account.parent_id = parent_id_arg AND person.id = linked_account.student_id ORDER BY person.last_name   LOOP

 RETURN NEXT children_names;
...
ALTER FUNCTION public.get_children_names(parent_id_arg text) OWNER TO postgres;
...
BEGIN
 ...
 FOR grade_item_id IN SELECT id FROM grade_item WHERE grading_system_id = grading_system_id_arg ORDER BY date LOOP
...
--
-- Name: get_grade_item_in_section(integer); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION get_grade_item_in_section(section_id_arg integer) RETURNS SETOF text
    LANGUAGE plpgsql
    AS $$DECLARE
 grading_system_id_var INTEGER;

 grade_item_id_var TEXT;
BEGIN
 FOR grading_system_id_var IN SELECT id FROM grading_system WHERE section_id = section_id_arg LOOP

   FOR grade_item_id_var IN SELECT grading_system.name || '#' || grade_item.id || '#' || grade_item.name || '#' || grade_item.total_score || '#' || grade_item.date FROM grade_item INNER JOIN grading_system ON (grading_system.id = grade_item.grading_system_id) WHERE grading_system_id = grading_system_id_var LOOP

    RETURN NEXT grade_item_id_var;    
...
ALTER FUNCTION public.get_grade_item_in_section(section_id_arg integer) OWNER TO postgres;
...
--
-- Name: get_unconfirmed_attendance(integer); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION get_unconfirmed_attendance(section_id_arg integer) RETURNS SETOF text
    LANGUAGE plpgsql
    AS $$DECLARE
 result TEXT;
BEGIN
 FOR result IN SELECT person.id|| '#' || person.last_name || ', ' || person.first_name || ' ' || person.middle_name || '#' || class_session.id || '#' || class_session.date FROM attendance INNER JOIN person ON (person.id = attendance.person_id AND person.type = attendance.person_type) INNER JOIN class_session ON (class_session.id = attendance.session_id) WHERE class_session.section_id = section_id_arg AND attendance.confirmed = false AND person.type = 'STUDENT' AND class_session.date = date(now()) LOOP
  RETURN NEXT result;
 END LOOP;
END;$$;


ALTER FUNCTION public.get_unconfirmed_attendance(section_id_arg integer) OWNER TO postgres;
...
CREATE FUNCTION grade_item_entry_information(grade_item_entry_id_arg integer) RETURNS text
 ...
 student_fname_output TEXT;

 student_mname_output TEXT;

student_lname_output TEXT;
...
--
-- Name: have_attended(text, integer); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION have_attended(student_id_arg text, session_id_arg integer) RETURNS boolean
    LANGUAGE plpgsql
    AS $$DECLARE
 result BOOLEAN;
BEGIN
 IF student_id_arg IN (SELECT person_id FROM attendance WHERE session_id = session_id_arg) THEN
  RETURN TRUE;
 ELSE
  RETURN FALSE;
 END IF;
...
ALTER FUNCTION public.have_attended(student_id_arg text, session_id_arg integer) OWNER TO postgres;

--
-- Name: is_confirmed(text, integer); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION is_confirmed(student_id_arg text, session_id_arg integer) RETURNS boolean
    LANGUAGE plpgsql
    AS $$DECLARE
 result BOOLEAN;
BEGIN
 SELECT INTO result confirmed FROM attendance WHERE session_id = session_id_arg AND person_id = student_id_arg;
 RETURN result;
END;$$;


ALTER FUNCTION public.is_confirmed(student_id_arg text, session_id_arg integer) OWNER TO postgres;
...
--
-- Name: populateclassrecord(bigint); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION populateclassrecord(grade_system_id_arg bigint) RETURNS SETOF text
    LANGUAGE plpgsql
    AS $$DECLARE

    student_grade_output TEXT;

    grade_item_id_output integer;
...
BEGIN
    ...
    SELECT INTO section_id_output section_id FROM grading_system where id = grade_system_id_arg;

    FOR student_id_output IN (SELECT enrolled(section_id_output)) LOOP

      student_grade_output = '';

      SELECT INTO name_output person.last_name || ', ' || person.first_name|| ' '||person.middle_name FROM person WHERE person.id = student_id_output;

      student_grade_output = student_grade_output || name_output;

      FOR grade_item_id_output IN (SELECT id from grade_item WHERE grading_system_id = grade_system_id_arg ORDER BY date) LOOP

        SELECT INTO score_output '#' || grade_item_entry.score FROM grade_item_entry WHERE person_id = student_id_output AND grade_item_id = grade_item_id_output;

        student_grade_output = student_grade_output || score_output;

      END LOOP;

      RETURN NEXT student_grade_output;
...
ALTER FUNCTION public.populateclassrecord(grade_system_id_arg bigint) OWNER TO postgres;

--
-- Name: FUNCTION populateclassrecord(grade_system_id_arg bigint); Type: COMMENT; Schema: public; Owner: postgres
--

COMMENT ON FUNCTION populateclassrecord(grade_system_id_arg bigint) IS 'Accepts: grading system id (integer)

Returns: list of students and their scores for each grade item belonging to that grade system ';


--
-- Name: recent_grade_item_added(integer); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION recent_grade_item_added(grading_system_id_arg integer) RETURNS integer
    LANGUAGE plpgsql
    AS $$DECLARE
 result INTEGER;
BEGIN
 SELECT INTO result max(id) FROM grade_item WHERE grading_system_id = grading_system_id_arg;
 RETURN result;
END$$;


ALTER FUNCTION public.recent_grade_item_added(grading_system_id_arg integer) OWNER TO postgres;
...
CREATE FUNCTION standing_grade(student_id_arg text, section_id_arg integer) RETURNS double precision
 ...
 grade_item_entry_sum_score DOUBLE PRECISION;
 final_grade DOUBLE PRECISION;
 grading_system_id_var INTEGER;
 grading_system_weight_var DOUBLE PRECISION;
 grade_item_id_var INTEGER;
...
BEGIN
 ...
 FOR grading_system_id_var, grading_system_weight_var IN SELECT id, weight FROM grading_system WHERE section_id = section_id_arg LOOP
  ...
  grade_item_entry_sum_score = 0;
  FOR grade_item_id_var, grade_item_total_score_var IN SELECT id, total_score FROM grade_item WHERE grading_system_id = grading_system_id_var LOOP
   FOR grade_item_entry_score_var IN SELECT score FROM grade_item_entry WHERE grade_item_id = grade_item_id_var AND person_id = student_id_arg AND person_type = 'STUDENT' LOOP
    grade_item_entry_sum_score = grade_item_entry_sum_score + grade_item_entry_score_var;
    grade_item_sum_total_score = grade_item_sum_total_score + grade_item_total_score_var;
   END LOOP;
  END LOOP;
  IF (grade_item_sum_total_score > 0) THEN
   final_grade = final_grade + ((grade_item_entry_sum_score/grade_item_sum_total_score) * grading_system_weight_var);
  END IF;
 END LOOP;
 RETURN final_grade;
END;$$;
...
ALTER FUNCTION public.standing_grade(student_id_arg text, section_id_arg integer) OWNER TO postgres;
...
BEGIN
 ...
 FOR session_id_output in SELECT id FROM class_session  WHERE section_id = section_id_arg AND status = 'DISMISSED' LOOP
...
INSERT INTO attendance VALUES (true, 19, '2009-1625', 'STUDENT', '2012-10-16 03:00:50');
INSERT INTO attendance VALUES (true, 20, '2010-7171', 'STUDENT', '2012-10-16 21:42:24');
INSERT INTO attendance VALUES (true, 21, '2010-7171', 'STUDENT', '2012-10-17 11:03:52');
INSERT INTO attendance VALUES (true, 21, '2009-1625', 'STUDENT', '2012-10-17 11:03:57');
INSERT INTO attendance VALUES (true, 22, '2009-1625', 'STUDENT', '2012-10-17 16:58:21');
INSERT INTO attendance VALUES (true, 23, '2009-1625', 'STUDENT', '2012-10-17 16:58:48');
INSERT INTO attendance VALUES (true, 24, '2010-7171', 'STUDENT', '2012-10-17 17:15:34');
INSERT INTO attendance VALUES (true, 24, '2009-1625', 'STUDENT', '2012-10-17 17:20:50');
INSERT INTO attendance VALUES (true, 25, '2010-7171', 'STUDENT', '2012-10-17 17:38:25');
INSERT INTO attendance VALUES (true, 25, '2009-1625', 'STUDENT', '2012-10-17 17:38:37');
INSERT INTO attendance VALUES (true, 26, '2010-7171', 'STUDENT', '2012-10-17 18:54:59');
INSERT INTO attendance VALUES (true, 27, '2010-7171', 'STUDENT', '2012-10-17 19:29:02');
INSERT INTO attendance VALUES (true, 26, '2009-1625', 'STUDENT', '2012-10-17 23:57:35');
INSERT INTO attendance VALUES (true, 28, '2009-1625', 'STUDENT', '2012-10-18 01:08:30');
INSERT INTO attendance VALUES (true, 28, '2010-7171', 'STUDENT', '2012-10-18 01:08:36');
INSERT INTO attendance VALUES (true, 29, '2009-1625', 'STUDENT', '2012-10-18 04:16:43');
INSERT INTO attendance VALUES (true, 30, '2010-7171', 'STUDENT', '2012-10-18 04:21:00');
INSERT INTO attendance VALUES (true, 30, '2009-1625', 'STUDENT', '2012-10-18 04:21:07');
INSERT INTO attendance VALUES (true, 31, '2010-7171', 'STUDENT', '2012-10-18 08:43:50');
INSERT INTO attendance VALUES (true, 33, '2009-1625', 'STUDENT', '2012-10-18 12:27:01');
INSERT INTO attendance VALUES (true, 34, '2010-7171', 'STUDENT', '2012-10-18 13:25:32');
INSERT INTO attendance VALUES (true, 35, '2010-7171', 'STUDENT', '2012-10-20 14:10:05');
INSERT INTO attendance VALUES (true, 35, '2009-1625', 'STUDENT', '2012-10-20 14:10:12');
INSERT INTO attendance VALUES (true, 36, '2010-7171', 'STUDENT', '2012-10-20 16:49:20');
INSERT INTO attendance VALUES (true, 37, '2009-1625', 'STUDENT', '2012-10-21 21:04:15');
INSERT INTO attendance VALUES (true, 38, '2010-7171', 'STUDENT', '2012-10-21 21:55:42');
INSERT INTO attendance VALUES (false, 31, '2009-1625', 'STUDENT', '2012-10-18 08:43:39');
...
INSERT INTO class_session VALUES (21, 3, 'DISMISSED', '2012-10-17');
INSERT INTO class_session VALUES (22, 3, 'DISMISSED', '2012-10-17');
INSERT INTO class_session VALUES (23, 3, 'DISMISSED', '2012-10-17');
INSERT INTO class_session VALUES (17, 1, 'DISMISSED', '2012-10-13');
INSERT INTO class_session VALUES (24, 3, 'DISMISSED', '2012-10-17');
INSERT INTO class_session VALUES (25, 3, 'DISMISSED', '2012-10-17');
INSERT INTO class_session VALUES (27, 1, 'DISMISSED', '2012-10-17');
INSERT INTO class_session VALUES (26, 3, 'DISMISSED', '2012-10-17');
INSERT INTO class_session VALUES (28, 3, 'DISMISSED', '2012-10-18');
INSERT INTO class_session VALUES (29, 3, 'DISMISSED', '2012-10-18');
INSERT INTO class_session VALUES (30, 3, 'DISMISSED', '2012-10-18');
INSERT INTO class_session VALUES (31, 3, 'DISMISSED', '2012-10-18');
INSERT INTO class_session VALUES (32, 3, 'DISMISSED', '2012-10-18');
INSERT INTO class_session VALUES (33, 3, 'DISMISSED', '2012-10-18');
INSERT INTO class_session VALUES (34, 3, 'DISMISSED', '2012-10-18');
INSERT INTO class_session VALUES (35, 3, 'DISMISSED', '2012-10-20');
INSERT INTO class_session VALUES (36, 3, 'DISMISSED', '2012-10-20');
...
INSERT INTO grade_item VALUES (186, 120, 'Assignment 2', 50, '2012-10-20 17:10:55');
INSERT INTO grade_item VALUES (187, 120, 'Assignment 3', 50, '2012-10-20 17:11:02');
INSERT INTO grade_item VALUES (181, 141, 'Quiz 2', 30, '2012-10-20 17:09:38');
INSERT INTO grade_item VALUES (199, 141, 'Quiz 3', 50, '2012-10-21 08:08:36');
INSERT INTO grade_item VALUES (200, 141, 'Quiz 0', 30, '2012-10-21 08:11:37');
INSERT INTO grade_item VALUES (205, 147, 'Prelim', 65, '2012-10-21 21:15:59');
INSERT INTO grade_item VALUES (213, 141, 'Quiz on Fags', 100, '2012-10-22 07:14:33');
INSERT INTO grade_item VALUES (214, 141, 'Quiz on You Know', 30, '2012-10-22 07:15:33');
INSERT INTO grade_item VALUES (216, 120, 'Assignment on Matrices', 200, '2012-12-21 07:24:52');
INSERT INTO grade_item VALUES (217, 117, 'Quiz on Statements', 200, '2012-10-22 07:30:50');
...
INSERT INTO grade_item_entry VALUES (362, 199, 51, '2010-7171', 'STUDENT');
INSERT INTO grade_item_entry VALUES (363, 199, 32, '2009-1625', 'STUDENT');
INSERT INTO grade_item_entry VALUES (339, 187, 45, '2009-1625', 'STUDENT');
INSERT INTO grade_item_entry VALUES (338, 187, 43, '2010-7171', 'STUDENT');
INSERT INTO grade_item_entry VALUES (325, 180, 51, '2009-1625', 'STUDENT');
INSERT INTO grade_item_entry VALUES (324, 180, 90, '2010-7171', 'STUDENT');
INSERT INTO grade_item_entry VALUES (375, 205, 43, '2009-1625', 'STUDENT');
INSERT INTO grade_item_entry VALUES (374, 205, 42, '2010-7171', 'STUDENT');
INSERT INTO grade_item_entry VALUES (337, 186, 70, '2009-1625', 'STUDENT');
INSERT INTO grade_item_entry VALUES (336, 186, 32, '2010-7171', 'STUDENT');
INSERT INTO grade_item_entry VALUES (326, 181, 45, '2010-7171', 'STUDENT');
INSERT INTO grade_item_entry VALUES (333, 184, 30, '2009-1625', 'STUDENT');
INSERT INTO grade_item_entry VALUES (364, 200, 0, '2010-7171', 'STUDENT');
INSERT INTO grade_item_entry VALUES (390, 213, 0, '2010-7171', 'STUDENT');
INSERT INTO grade_item_entry VALUES (391, 213, 0, '2009-1625', 'STUDENT');
INSERT INTO grade_item_entry VALUES (392, 214, 0, '2010-7171', 'STUDENT');
INSERT INTO grade_item_entry VALUES (393, 214, 0, '2009-1625', 'STUDENT');
INSERT INTO grade_item_entry VALUES (396, 216, 0, '2010-7171', 'STUDENT');
INSERT INTO grade_item_entry VALUES (397, 216, 0, '2009-1625', 'STUDENT');
INSERT INTO grade_item_entry VALUES (399, 217, 21, '2010-7171', 'STUDENT');
INSERT INTO grade_item_entry VALUES (398, 217, 200, '2009-1625', 'STUDENT');
INSERT INTO grade_item_entry VALUES (400, 218, 0, '2009-1625', 'STUDENT');
INSERT INTO grade_item_entry VALUES (401, 218, 0, '2010-7171', 'STUDENT');
INSERT INTO grade_item_entry VALUES (403, 219, 10, '2010-7171', 'STUDENT');
...
INSERT INTO grading_system VALUES (25, 97, 'Exam', 2);
